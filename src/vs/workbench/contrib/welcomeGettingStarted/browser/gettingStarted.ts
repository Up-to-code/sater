...
import { applySaterInterfaceLanguage, SATER_INTERFACE_LANGUAGE_SETTING, SATER_LANGUAGE_ONBOARDING_COMPLETE, SaterInterfaceLanguage } from '../../sater/browser/saterLocale.js';
...
export class GettingStartedPage extends EditorPane {
	...
	private async buildCategoriesSlide(preserveFocus?: boolean) {
		...
		if (this.productService.applicationName === 'sater') {
			this.buildSaterCategoriesSlide(showOnStartupCheckbox.domNode, showOnStartupLabel);
			return;
		}
	...
	private buildSaterCategoriesSlide(showOnStartupCheckbox: HTMLElement, showOnStartupLabel: HTMLElement): void {
		this.container.classList.add('saterWelcome');
		if (!this.storageService.getBoolean(SATER_LANGUAGE_ONBOARDING_COMPLETE, StorageScope.APPLICATION, false)) {
			this.buildSaterLanguageOnboarding(showOnStartupCheckbox, showOnStartupLabel);
			return;
		}
		...
		const actions = $('.sater-welcome-actions', {},
			action(
				copy.openProject,
				copy.openProjectDescription,
				'codicon-folder-opened',
				'saterCommand:workbench.action.files.openFolder'
			),
			action(
				copy.cloneRepository,
				copy.cloneRepositoryDescription,
				'codicon-repo-clone',
				'saterCommand:git.clone'
			),
			action(
				copy.connectSsh,
				copy.connectSshDescription,
				'codicon-remote',
				'saterCommand:workbench.action.remote.showMenu'
			),
			action(
				copy.connectGithub,
				copy.connectGithubDescription,
				'codicon-github-inverted',
				'openLink:https://github.com/login'
			)
		);
		...
		const footer = $('.sater-welcome-footer', {},
			$('p.showOnStartup', {}, showOnStartupCheckbox, showOnStartupLabel)
		);
		...
		reset(this.categoriesSlide,
			$('.sater-welcome-shell', {},
				brand,
				actions,
				$('.sater-welcome-recents', {}, recentElement),
				footer
			)
		);
	...
	private buildSaterLanguageOnboarding(showOnStartupCheckbox: HTMLElement, showOnStartupLabel: HTMLElement): void {
		const selectLanguage = async (language: SaterInterfaceLanguage) => {
			this.storageService.store(SATER_LANGUAGE_ONBOARDING_COMPLETE, true, StorageScope.APPLICATION, StorageTarget.USER);
			applySaterInterfaceLanguage(language);
			await this.configurationService.updateValue(SATER_INTERFACE_LANGUAGE_SETTING, language, ConfigurationTarget.USER);
			this.buildSaterCategoriesSlide(showOnStartupCheckbox, showOnStartupLabel);
		};

		const languageButton = (language: SaterInterfaceLanguage, label: string, description: string) => {
			const button = $('button.sater-language-choice', {
				type: 'button',
				lang: language,
			...
			this.categoriesSlideDisposables.add(addDisposableListener(button, 'click', () => selectLanguage(language)));
			return button;
		};
