...
export class StartupPageRunnerContribution extends Disposable implements IWorkbenchContribution {
	...
	private didHideSaterActivityBar = false;
	...
	constructor(
		...
		this._register(this.contextService.onDidChangeWorkbenchState(state => {
			if (this.didHideSaterActivityBar && state !== WorkbenchState.EMPTY) {
				this.didHideSaterActivityBar = false;
	...
	private async openGettingStarted(showTelemetryNotice?: boolean) {
		...
		if (startupEditorTypeID === gettingStartedInputTypeId) {
			...
			if (this.productService.applicationName === 'sater' && this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				this.didHideSaterActivityBar = true;
	...
	private tryShowOnboarding(): void {
		if (this.productService.applicationName === 'sater') {
			return; // Sater owns its first-run experience in the Welcome editor.
		}
